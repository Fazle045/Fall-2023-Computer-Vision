...
def run_main(FLAGS):
    ...
    # Import CIFAR10 dataset for training from torchvision datasets
    train_dataset = datasets.CIFAR10(
        root="torch_datasets", train=True, transform=transform, download=True
    )
    # Define dataloader for training using cifar10 train datasets
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=FLAGS.batch_size, shuffle=True
    )
    # Import CIFAR10 dataset for testing from torchvision datasets
    test_dataset = datasets.CIFAR10(
        root="torch_datasets", train=False, transform=transform, download=True
    )
    # Define dataloader for testing using cifar10 test datasets
